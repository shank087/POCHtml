...
public class SeleniumWD {
	...
	public void setBrowser(String BrowserType) {
		...
		try {
			 ...
			 * 
			 * String filename = listofFiles[i].getName();
			 * if(filename.contains("gecko")||filename.contains("GECKO")||filename.contains(
			 * "Gecko")){ filename = listofFiles[i].toString(); ffDriverPath
			 * =filename.toString();
			 * 
			 * break; } } System.setProperty("webdriver.gecko.driver", ffDriverPath);
			 * 
			 * DesiredCapabilities capabilities = DesiredCapabilities.firefox();
			 * capabilities.setCapability("marionette", true); D8 = new
			 * FirefoxDriver(capabilities);
			 * 
			 * //D8 = new FirefoxDriver(); D8.manage().timeouts().implicitlyWait(30,
			 * TimeUnit.SECONDS); D8.manage().window().maximize(); browser="FF"; break;
			 */
	...
	public void findExecTestscript(String TestSuite, String TestScript, String ObjectRepository) throws Exception {
		try {
			...
			String htmlname1 = rp + "Log" + "Test_Suite_" + strTimeStamp + "_" + browser + ".html";
					...
					case Cell.CELL_TYPE_STRING:
						...
						if (runscript == true) {
							...
							try {
								...
								LOGGER.log(Level.SEVERE,
										"Invalid File: " + ScriptName + "is not available.Give the correct TestScript");
							...
							if ((exeStatus.equalsIgnoreCase("Failed"))
								...
								// 2016-03-28 A pass of a reference previous HTML was corrected in a relative
								// path.
...

	{
		...
		String htmlname = reportsPath + "Log" + "\\" + TCNm[0] + "\\" + strTimeStamp + ".html";

		reportsFolder = reportsPath + "Log" + "\\" + TCNm[0] + "\\";

		// 2016-03-28 A pass of a reference previous HTML was corrected in a relative
		// path.
		DetailReport = ".\\Log" + "\\" + TCNm[0] + "\\" + strTimeStamp + ".html";
		...
		try {
			if (devFlag == 0) {
				...
				if (loopflag == 0) {
					...
					filenamer = screenReport + Sname + "\\" + Sname + "_" + browser + "_" + screenshotflag + "_rowno_"
							+ (j + 1) + "_" + strTime + ".png";

				} else {

					filenamer = screenReport + Sname + "\\" + Sname + "_" + browser + "_" + screenshotflag + "_loop_"
							+ (loopcnt[loopsize] + 1) + "_rowno_" + (j + 1) + "_" + strTime + ".png";
				}
	...
	public static void update_Report(String Res_type) throws IOException {
		try {
			...
			if (Res_type.startsWith("executed")) {
				...
				bw.write(
						"<TR COLS=6><th colspan= 6 BGCOLOR=#EEEEEE WIDTH=5%><FONT FACE='WINGDINGS 2' SIZE=3 COLOR=RED><div align=left>X </FONT><FONT FACE=VERDANA SIZE=2 COLOR = RED>Error Occurred in  test step number. "
								+ (j + 1) + ". " + "Property not supported by Open2Test</div></th></FONT></TR>");
	...
	public static void update_Report(String Res_type, Exception e) throws IOException {
		try {
			...
			if (Res_type.startsWith("failed")) {
				...
				bw.write(
						"<TR COLS=6><th colspan= 6 BGCOLOR=#EEEEEE WIDTH=5%><FONT FACE='WINGDINGS 2' SIZE=3 COLOR=RED><div align=left>X </FONT><FONT FACE=VERDANA SIZE=2 COLOR = RED>Error Occurred in  test step number "
								+ (j + 1) + ". Description: Keyword not supported by Open2Test</div></th></FONT></TR>");
	...
	// the report
	{
		try {
			...
			if (Res_type.startsWith("executed") && ObjectSet.equalsIgnoreCase("page")) {
				...
				bw.write(
						"<TR COLS=6><th colspan= 6 BGCOLOR=#EEEEEE WIDTH=5%><FONT FACE='WINGDINGS 2' SIZE=3 COLOR=RED><div align=left>X </FONT><FONT FACE=VERDANA SIZE=2 COLOR = RED>Error Occurred in test step number "
								+ (j + 1) + ". This property is not supported by Open2Test</div></th></FONT></TR>");
			}
		...
		try {
			...
			if (objFoundFlag == 1) {
				...
				case "text":
					...
					} else if (objectType.equalsIgnoreCase("textelement") || objectType.equalsIgnoreCase("element")) {
		...
		try {
			...
			case "endloop":
				...
				if (loopcnt[loopsize] == loopcount[loopsize]) {
					update_Report("loop" + " End of Loop : " + (loopsize + 1) + " : Loop count : " + loopcnt[loopsize]);
					loopsize = loopsize - 1;
					if (loopsize >= 0)
		...
		if (browserName.equalsIgnoreCase("ff")) {
			...
			case "upload":
				try {
					...
					Thread.sleep(2000);
					Runtime.getRuntime().exec(execpath + " 2 upload " + cCellData + " " + browserName.toLowerCase());
					update_Report("executed");
				} catch (IOException e) {
			...
			case "abortupload":
				try {
					...
					Runtime.getRuntime().exec(execpath + " 2 closeupload " + "abort" + " " + browserName.toLowerCase());
			...
			case "download":
				try {
					...
					Runtime.getRuntime().exec(execpath + " 2 download " + ObjectSet + " " + browserName.toLowerCase()
			...
			if (!(lastSelecteddateJQ == setdd && lastSelectedmonthJQ == setmm && lastSelectedyearJQ == setyyyy)) {
				...
				for (String strtitleMonth : setObj.envtitleMonth) {
					...
					try {
						...
						try {
							titleMonth = D8.findElementByXPath("//select[contains(@class,'" + strtitleMonth + "')]");
				...
				for (String strtitleYear : setObj.envtitleYear) {
					...
					try {
						...
						try {
							...
							titleYearnum = Integer.parseInt(new Select(titleYear).getFirstSelectedOption().getText());
							usedTitleYear = strtitleYear;
							usedTitletag = "select";
							objectfound = 1;
				...
				if (isDateNotSelected == 0) {
					if (setyyyy == titleYearnum && setmm == monthnum1) {
						...
						try {
							...
							List<WebElement> daystoClick = D8.findElementsByXPath("//a[contains(@class,'" + locatorPrev
									+ "')]" + "/following::a[contains(text(),'" + setdd + "')]");
							...
							for (int i2 = 0; i2 < daystoClick.size(); i2++) {
								...
								} else if (Integer.parseInt(daystoClick.get(i2).getText()) == setdd
										&& (daystoClick.get(i2).getAttribute("class").equalsIgnoreCase(dateClass)
		...
		case "boot":
			...
			if (setyyyy > titleYearnum && setmm > monthnum1) {
				try {
					...
					while (!((titleMonthString.toLowerCase().contains(monthpart.toLowerCase())
							|| titleMonthString.toLowerCase().contains(monthpartjap)) && (setyyyy == titleYearnum)));
				} catch (Exception e) {
					update_Report("invaliddate");
